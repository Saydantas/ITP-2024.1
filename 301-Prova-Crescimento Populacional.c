...
apenas para as cidades cuja taxa de crescimento da cidade A é maior do 
que a taxa de crescimento da cidade B, portanto, previamente já separou 
para você apenas os casos de teste que tem a taxa de crescimento maior 
para a cidade A. Sua tarefa é construir um programa que apresente o tempo 
...
de casos de teste (1 ≤ T ≤ 3000). Cada caso de teste contém 4 números: dois 
...
resultará em 1025 pessoas. Além disso, não utilize variáveis de precisão simples 
...
int main() {
    ...
    for (int i = 0; i < T; i++) {
        ...
        // Enquanto a população de A for menor ou igual a população de B
        while (PA <= PB) {
            // Calcular o crescimento populacional de cada cidade
            PA += (int)(PA * G1 / 100);
            PB += (int)(PB * G2 / 100);

            // Incrementar o número de anos
            anos++;

            // Verificar se ultrapassou o limite de 100 anos
            if (anos > 100) {
        ...
        // Se a população de A ultrapassou a população de B, imprimir o número de anos
        if (anos <= 100) {
            printf("%d anos\n", anos);
        }
    }
